(() => {
  ...
  const fields = {
    ...
    action: $('operation-action'), active: $('operation-active'),
  };
  ...
  const clearForm = () => { fields.id.value = ''; fields.name.value = ''; fields.instruction.value = ''; fields.action.value = ''; fields.active.checked = true; fields.id.focus(); };
  const fillForm = (item) => { fields.id.value = item.operation_id || ''; fields.name.value = item.name || ''; fields.instruction.value = item.instruction || ''; fields.action.value = (item.actions || [])[0] || ''; fields.active.checked = item.active !== false; window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  function render(items) {
    ...
    items.forEach((item) => {
      ...
      const meta = document.createElement('p'); meta.className = 'operation-meta'; meta.textContent = `判定方法: ${(item.actions || [])[0] || '-'} / ${item.active === false ? 'inactive' : 'active'} / version: ${item.version || 1}`;
  ...
  async function saveOperation() {
    ...
    const payload = { name: fields.name.value.trim(), instruction: fields.instruction.value.trim(), actions: fields.action.value ? [fields.action.value] : [], active: fields.active.checked };
    if (!/^OP\d{3,}$/.test(id) || !payload.name || !payload.instruction || payload.actions.length !== 1) { setStatus('Enter an OP001-style ID, name, instruction, and判定方法.', true); return; }
